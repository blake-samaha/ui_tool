...
export const tier01Steps: WizardStep[] = [
  ...
  },
  {
    id: 'objects',
    title: 'Objects',
    uiSchema: { fields: [ { kind: 'array', path: 'objects', label: 'Objects', addLabel: 'Add', removeLabel: 'Remove', render: 'table', columns: ['name', 'objectId', 'space', 'viewExternalId', 'containerExternalId'], pageSize: 20, actions: [ { label: 'Open XX', hrefTemplate: '/tierXX?step=basics&projectId={{projectId}}&moduleId={{moduleId}}&objectId={{objectId}}' } ], item: { kind: 'group', fields: [ { kind: 'text', id: 'name', label: 'Name', placeholder: 'e.g., Well' }, { kind: 'text', id: 'objectId', label: 'Object ID', placeholder: 'e.g., workOrder', help: 'LowerCamelCase' }, { kind: 'multiselect', id: 'implementsCore', label: 'Implements Core/Industry Views', placeholder: 'Search, e.g., asset, event', options: ALL_KNOWN_VIEWS.map((o) => ({ ...o, group: o.group ?? (o.label.startsWith('Cognite') ? 'Core/Process' : 'Other') })) }, { kind: 'text', id: 'space', label: 'Space', placeholder: 'e.g., sp_my_project_space' }, { kind: 'text', id: 'viewExternalId', label: 'View External ID', placeholder: 'my_module:view:work_order:v1' }, { kind: 'text', id: 'containerExternalId', label: 'Container External ID', placeholder: 'my_module:container:work_order:v1' } ] } } ] }
  },
  {
    id: 'relationships',
    title: 'Relationships',
